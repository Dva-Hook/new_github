...
- Signed Arkose `/rtig/image` responses stay on the proxy route by default.
  Runner-direct image fetches are retained only as the experimental
  `--direct-challenge-images` mode because Arkose can bind these URLs to the
  challenge route and return `403` after an egress change.
